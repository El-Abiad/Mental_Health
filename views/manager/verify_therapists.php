                                    ...
                                    <table class="table table-hover align-middle mb-0">
                                        ...
                                        <tbody>
                                            <?php foreach ($rows as $t):
                                                ...
                                                $exp = !empty($t['ExpiryDate']) && strtotime($t['ExpiryDate']) < time();
                                            ?>
                                                <tr>
                                                    <td>
                                                    ...
                                                    <td><?= $t['ExpiryDate'] ? date('d M Y', strtotime($t['ExpiryDate'])) : '—' ?><?php if ($exp): ?> <span class="badge bg-danger">Expired</span><?php endif; ?></td>
                                                    <td><span class="badge bg-<?= $cls ?>"><?= ucfirst($lic) ?></span></td>
                                                    <td>
                                                        <?php if (!empty($t['LicenseId'])): ?>
                                                            <div class="d-flex gap-1 flex-wrap">
                                                                <?php if ($lic !== 'valid'): ?>
                                                                    <form method="POST" action="manager_run.php?action=verifyTherapists"><input type="hidden" name="action" value="verify"><input type="hidden" name="license_id" value="<?= $t['LicenseId'] ?>"><button class="btn btn-sm btn-success">Verify</button></form>
                                                                <?php endif; ?>
                                                                ...
                                                                <?php if ($lic !== 'revoked'): ?>
                                                                    <form method="POST" action="manager_run.php?action=verifyTherapists" onsubmit="return confirm('Revoke this license?')"><input type="hidden" name="action" value="revoke"><input type="hidden" name="license_id" value="<?= $t['LicenseId'] ?>"><button class="btn btn-sm btn-outline-danger">Revoke</button></form>
                                                                <?php endif; ?>
                                                            </div>
    ...
    <div class="modal fade" id="renewModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="POST" action="manager_run.php?action=verifyTherapists">
