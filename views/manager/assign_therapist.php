...
<div class="container py-4" style="max-width:860px">
  <div class="d-flex align-items-center gap-2 mb-4">
    ...
    <h3 class="fw-bold mb-0">Assign Therapist to Patient</h3>
  </div>
  ...
  <div class="card border-0 shadow-sm">
    <div class="card-body p-4">
      <form method="POST" action="manager_run.php?action=assignTherapist">
        <div class="mb-3">
          <label class="form-label fw-semibold">Patient</label>
          <select name="patient_id" class="form-select" required>
            <option value="">— Select patient —</option>
            <?php foreach ($patients as $p): ?>
              <option value="<?= $p['Id'] ?>"><?= htmlspecialchars($p['FullName']) ?> (<?= htmlspecialchars($p['Email']) ?>)</option>
            <?php endforeach; ?>
          </select>
        </div>
        <div class="mb-3">
          <label class="form-label fw-semibold">Therapist</label>
          <select name="therapist_id" id="therapistSelect" class="form-select" required>
            <option value="">— Select therapist —</option>
            <?php foreach ($therapists as $t): ?>
              <option value="<?= $t['Id'] ?>" data-snoozed="<?= $t['IsSnoozed'] ? '1' : '0' ?>">
                <?= htmlspecialchars($t['FullName']) ?><?= $t['Specialization'] ? ' — ' . htmlspecialchars($t['Specialization']) : '' ?><?= $t['IsSnoozed'] ? '  (Snoozed)' : '' ?>
              </option>
            <?php endforeach; ?>
          </select>
          <div id="snoozeWarning" class="form-text text-warning d-none">⚠️ This therapist is snoozed and not accepting new patients.</div>
        </div>
        <div class="mb-3">
          ...
          <div class="form-text">Double-booking is prevented automatically</div>
        </div>
        <div class="mb-4">
          <label class="form-label fw-semibold">Session Fee (EGP)</label>
        ...
        <div class="d-flex gap-2">
          ...
          <a href="manager_run.php?action=dashboard" class="btn btn-outline-secondary">Cancel</a>
        </div>
      </form>
    </div>
  </div>
...
document.getElementById('therapistSelect').addEventListener('change', function () {
  document.getElementById('snoozeWarning').classList.toggle('d-none', this.options[this.selectedIndex].dataset.snoozed !== '1');
});
</script>
